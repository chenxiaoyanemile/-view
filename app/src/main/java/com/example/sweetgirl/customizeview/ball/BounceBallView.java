...
public class BounceBallView extends View{
    ...
    private final static String TAG = "BallView";
    ...
    private boolean isRandomBallPath = true;
    private boolean isRandomColor = true;
    private boolean isRandomRadius = true;
    ...
    private boolean isPhysicsMode = true;
    ...
    private android.view.animation.Interpolator defaultInterpolator = new LinearInterpolator();
    ...
    public BounceBallView(Context context, AttributeSet attrs, int deftyle)
    {
        super(context, attrs, deftyle);
    ...
    private void initData() {
        ...
        paints = new Paint[ballCount];
        for (int i = 0; i < paints.length; i++) {
            paints[i] = new Paint(Paint.ANTI_ALIAS_FLAG);
            paints[i].setColor(ballColor);
    ...
    public void start(final int duration) {
        post(new Runnable() {  //放入队列（保证view已加载完成）
            @Override
            public void run() {
                createAnim(duration); //检查内部有没有重复创建实例
                startAnim();
            }
        });
    }
            ...
            translateAnim[index].addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    if (dealFromAlphaAnim(translateFraction[index]) != -1) {
                        paints[index].setAlpha(dealFromAlphaAnim(translateFraction[index]));
                    } else if (dealToAlphaAnim(translateFraction[index]) != -1) {
                        paints[index].setAlpha(dealToAlphaAnim(translateFraction[index]));
                    } else {
                        paints[index].setAlpha(255);
                    }
    ...
    private void makeRandom(int index) {
        ...
        if (isRandomColor) {  //不要在ondraw里再应用，会同时覆盖透明度通道，透明动画会失效
            ...
            paints[index].setColor(randomBallColors[index]);
        } else {
            paints[index].setColor(ballColor);
        }
